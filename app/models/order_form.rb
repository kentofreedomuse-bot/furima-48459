class OrderForm
  ...
  with_options allow_blank: true do |order_form|
    order_form.validates :postcode, presence: true, format: { with: /\A\d{3}-\d{4}\z/, message: '郵便番号はハイフンを含む正しい形式で入力してください' }
    order_form.validates :phone_number, presence: true, format: { with: /\A\d{10,11}\z/, message: '電話番号は10桁または11桁の数字で入力してください' }
  end
